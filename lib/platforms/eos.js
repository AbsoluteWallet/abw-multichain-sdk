...
class EosPlatform {
    ...
    genTxObj(txS) {
        ...
        for (let i = 0; i < txS.length; i++) {
            ...
            const txValue = `${this.coinFormatStr(txObj.tx.value, coin['satoshi'] || 6)} ${coin.assetName}`;

            dest.push({
                account: coin.contract,
                name: 'transfer',
                authorization: [{
                    actor: txObj.sourceItem.address,
                    permission: 'active',
                }],
                data: {
                    from: txObj.sourceItem.address,
    ...
    async buildTransaction(node, txObj, txName = 'transfer') {
        ...
        if (txName === 'transfer') {
            txActions = {
                actions: txObj.actions
            };
        }

        // TODO: add support for staking
