...
const { Binance, EtherPlatform, AptosPlatform, Bitcoin, PolkaPlatform,
    RadixPlatform, SolanaPlatform, TerraPlatform, TronPlatform, EosPlatform,
    SuiPlatform } = require('./../platforms');
const { coinFormat, coinFormatStr } = require('./utils');
...
class Blockchain  {
    constructor(apiClient, testNodes = false) {
        ...
        /** EOS (EOS, WAX) **/
        const wax = new EosPlatform('wax', apiClient);
        this.platformMap.set('wax', wax);

        const eos = new EosPlatform('eos', apiClient);
        this.platformMap.set('eos', eos);

        /** SUI **/
        const sui = new SuiPlatform('wax', apiClient);
        this.platformMap.set('sui', sui);
    ...
    async registerWallet(chain, mnemonic, index) {
        ...
        if (node.platform !== 'polka') {
            platform = await this.getPlatform(node.platform);
            if (!platform) return;
        } else {
            platform = PolkaPlatform;
        }
    ...
    async convertPolkaAddress(address, chainId) {
        ...
        // if (!platform) return;
        return PolkaPlatform.convertAddress(address, chainId);
    }
    ...
    async addressFromKey(chain, key) {
        ...
        const platform = await this.getPlatform(platformName);
        if (!platform) return;
        const address =  await platform.addressFromKey(key, chain);
        const node = this.nodesMap.get(chain);
    ...
    async checkAddress(coinName, address) {
        ...
        if (coin) {
            ...
            const platform = await this.getPlatform(node.platform);
            if (!platform) return;
            return platform.checkAddress(address, node.name);
        } else {
