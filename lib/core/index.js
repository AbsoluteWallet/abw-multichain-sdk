 ...
 * @param {string} [mnemonic] if omitted - random mnemonic will be generated (mnemonic is mandatory when passing a USER)
 * @param {object} [user] pass previously generated user with wallets to add wallets for new blockchains
 * @returns {Promise<object>} all wallets in one object
 */
async function generateAllWallets(mnemonic, user) {
    let existingWallets = {};
    let mnemo;
    let wallets = {};

    if (user) {
        if (!mnemonic) throw new Error('Need to provide old mnemonic for User');
        mnemo = mnemonic;
        existingWallets = user.wallet;
    }  else {
        mnemo = mnemonic || (await blockchain.generateMnemonic());
        wallets.mnemonic = mnemo;
    }

    const platforms = new Set();

    platforms.add('eos'); // skip wallet generation for eos
    platforms.add('wax'); // skip wallet generation for wax
    ...
    for (const net of networks) {
        ...
        if (existingWallets[net.platform]) {
            wallets[net.platform] = existingWallets[net.platform];
            continue;
        }

        pfms.push(net.platform);
        // const prefix = net.platform;

        let netId = net.platform === 'polka' ? 'polka' : net.id;
        // let wallet = await blockchain.registerWallet(net.id, mnemo);
        promises.push(blockchain.registerWallet(netId, mnemo));
        // if (wallet) {
    ...
    for (let i = 0; i < pfms.length; i++) {
        let wallet = res[i];
        if (wallet) {
            wallets[pfms[i]] = {address: wallet.walletAddress, key: wallet.walletKey};
        }
    }
...
/**
 * Decode QR code from image (file or link or buffer)
 * @param {string|buffer} file (local file path or URL or buffer)
 * @returns {Promise<array<object>>} array of decoded texts and results coincidence (probability)
 */
async function qrCodeDecode(file) {
    return apiClient.qrCodeDecode(file);
}
